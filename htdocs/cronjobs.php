...
function make_graph($id_element, $settings) {
    ...
    foreach ($blackbox->modules as $mod => $module) {
        ...
        foreach ($records as $n => $datetime) {
            ...
            if (isset($hash[$mod][$rtime]))
                continue; //first one rules as its closer to the 00 mark
            $hash[$mod][$rtime] = $n;
        }
    }
    ...
    //set x and ymax
    if (!isset($settings['ymax']) or ! $settings['ymax'])
        $settings['ymax'] = 'auto';
    if (!isset($settings['ymin']) or ! $settings['ymin'])
        $settings['ymin'] = '0';
