...
while ($row = $db->fetch_row($result)) {
    ...
    if (!isset($page->tags["Pane::$panetag"]))
    ...
    if ($type == 'd') {
        ...
        if (isset($modules[$mod])) {
            ...
            $unit = $modules[$mod]->datapoints[$dp]->unit;
            if (!$unit)
                $unit = '&nbsp;';

            if (preg_match("/^\d+$/", $resolution) and preg_match("/^[\d.]+$/", $value))
                $value = number_format($value, (int) $resolution, '.', '');
