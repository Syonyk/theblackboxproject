...
class Blackbox {
    ...
    public function __construct($incldata = false) {
        ...
        foreach (scandir("modules") as $modname) {
            if ($modname[0] === '.' or $modname[0] === '_')
                continue;

            //invoke the module instance(s)
            if (!file_exists("modules/$modname/$modname.php"))
                die("Module $modname.php not found");
            require("modules/$modname/$modname.php");
            if (!class_exists($modname))
                die("Module class $modname not found in $modname.php");
            $this->modules[$modname] = new $modname;
            $profiler->add('Module configs all read');

            //load the datapoints data if requested
            if ($incldata)
                $this->modules[$modname]->load_data();
        }
    ...
    public function check_dbase($fix = false) {
        ...
        if ($this->dbase_has_been_checked)
            return false;
        ...
        //iterate each module
        foreach ($this->modules as $mod => $module) {
            if (!$module->get_setting('store_in_db'))
                continue;
            ...
            foreach ($module->datapoints as $label => $datapoint) {
                ...
                if (!isset($done[$label])) {
                    if ($fix) {
                        ...
                        $errors[] = "Column: $label ($table) missing";
                }
            }
            ...
            foreach ($module->datapoints as $label => $datapoint) {
                ...
                if (!isset($done[$label])) {
                    if ($fix) {
                        ...
                        $errors[] = "Column: $label ($daytable) missing";
                }
            }
        }
...
abstract class Module {
    ...
    public function __construct() {
        ...
        if (!file_exists($config))
            die("Module $modlabel config not found - $config");
        include($config);
    ...
    public function process_device() {
        ...
        if (!$data)
            ...
            if (!in_array(date("Y-m-d"), $this->datetimes['day']))
                $this->datetimes['day'][] = date("Y-m-d", strtotime($this->datetime));
            foreach ($this->datapoints as $label => $datapoint) {
                // PHP 7.2 breaks count on non-array types.  Work around.
                if (isset($data[$label]) && (is_array($data[$label]) || $data[$label] instanceof Countable)) {
                    $dcount = count($data[$label]);
                } else {
                    $dcount = 1;
                }

                if (isset($data[$label]) and $dcount) {
                    $datapoint->append($data[$label]);
                }
            }
    ...
    public function read_direct() {
        ...
        if (!$data)
            ...
            if (!in_array(date("Y-m-d"), $this->datetimes['day']))
                $this->datetimes['day'][] = date("Y-m-d", strtotime($this->datetime));
            foreach ($this->datapoints as $label => $datapoint) {
                if (isset($data[$label]) and count($data[$label]))
                    $datapoint->append($data[$label]);
            }
    ...
    protected function read_dbase() {
        ...
        if (!$this->settings['store_in_db'])
            return true;
        if ($this->dbase_has_been_read)
            return true;
        ...
        $data = array();
        foreach ($this->datapoints as $label => $datapoint) {
            if ($datapoint->store and $datapoint->interval == 'periodic')
                $data[$label] = array();
        }
        ...
        foreach ($this->datapoints as $label => $datapoint) {
            if (isset($data[$label]))
                $datapoint->set('periodic', $data[$label]);
        }
        ...
        $data = array();
        foreach ($this->datapoints as $label => $datapoint) {
            if ($datapoint->store and $datapoint->interval == 'day')
                $data[$label] = array();
        }
    ...
    protected function calc_derived() {
        ...
        foreach ($this->datapoints as $label => $datapoint) {
            ...
            //store
            if (is_array($data))
                $datapoint->set('periodic', $data);
            else
                $datapoint->set('day', $data);
        }
...
class Datapoint {
    ...
    public function __get($var) {
        if (isset($this->propertys[$var]))
            return $this->propertys[$var];
        if (isset($this->$var))
            return $this->$var;
        return false;
    }
