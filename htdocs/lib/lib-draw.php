...
class Draw {
    ...
    function multiline($x1, $y1, $x2, $y2, $opts) {
        ...
        if ($linewidth == 3) {
            ...
            $ax1 = cos(deg2rad($angle - 90)) * $offset + $x1;
            $ay1 = sin(deg2rad($angle - 90)) * $offset + $y1;
            $ax2 = cos(deg2rad($angle - 90)) * $offset + $x2;
            $ay2 = sin(deg2rad($angle - 90)) * $offset + $y2;
            $this->singleline($ax1, $ay1, $ax2, $ay2, $opts);

            $ax1 = cos(deg2rad($angle + 90)) * $offset + $x2;
            $ay1 = sin(deg2rad($angle + 90)) * $offset + $y2;
            $ax2 = cos(deg2rad($angle + 90)) * $offset + $x1;
            $ay2 = sin(deg2rad($angle + 90)) * $offset + $y1;
            $this->singleline($ax1, $ay1, $ax2, $ay2, $opts);
    ...
    function drawlinepchart($x1, $y1, $x2, $y2, $opts) {
        ...
        $angle = $this->getangle($x1, $y1, $x2, $y2);
        $points = array();
        $points[] = cos(deg2rad($angle - 90)) * $linewidth / 2 + $x1;
        $points[] = sin(deg2rad($angle - 90)) * $linewidth / 2 + $y1;
        $points[] = cos(deg2rad($angle + 90)) * $linewidth / 2 + $x1;
        $points[] = sin(deg2rad($angle + 90)) * $linewidth / 2 + $y1;
        $points[] = cos(deg2rad($angle + 90)) * $linewidth / 2 + $x2;
        $points[] = sin(deg2rad($angle + 90)) * $linewidth / 2 + $y2;
        $points[] = cos(deg2rad($angle - 90)) * $linewidth / 2 + $x2;
        $points[] = sin(deg2rad($angle - 90)) * $linewidth / 2 + $y2;
    ...
    function polygon($points, $opts) {
        ...
        for ($i = 0; $i <= count($points) - 1; $i = $i + 2) {
            if (isset($points[$i + 2]))
                $this->singleline($points[$i], $points[$i + 1], $points[$i + 2], $points[$i + 3], $opts);
            else
                $this->singleline($points[$i], $points[$i + 1], $points[0], $points[1], $opts);
        }
