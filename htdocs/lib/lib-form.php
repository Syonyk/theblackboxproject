...
class Form {
    ...
    function spambot_check() {
        ...
        if (strlen($intotal) > 10000)
            $spam_errs[] = 1;

        //urls and stray nasties
        if (preg_match("/bcc:|cc:|multipart|<script|<iframe|\[url|Content-Type:/i", $intotal))
            $spam_errs[] = 2;

        if (preg_match("~\b((http|https|ftp|gopher)://[a-zA-Z0-9]+)|([a-zA-Z][a-zA-Z0-9]+\.[a-zA-Z0-9]+\.[a-zA-Z]{2,4}(\.[a-zA-Z]{2,4})?)\b~", $intotal))
            $spam_errs[] = 3;
        elseif (preg_match("~\b(http|https|ftp|gopher)://[a-z0-9.]+\.[a-z]{2,6}\b~i", $intotal))
            $spam_errs[] = 3;
        elseif (preg_match("~\bwww\.[a-z0-9.]+\.[a-z]{2,6}\b~i", $intotal))
            $spam_errs[] = 3;

        //ip
        $ip = $_SERVER['REMOTE_ADDR'];
        if (!$this->isgoodipaddress($ip))
            $spam_errs[] = 4;
        ...
        if (($valival <> $code1) && ($valival <> $code2))
            $spam_errs[] = 5;
        ...
        //session
        if (!isset($_SESSION['valival3']) or $_SESSION['valival3'] <> 'SessionValid')
            $spam_errs[] = 7;
    ...
    function makeradiogroup($radio) {
        ...
        if (!isset($radio['selected']))
            $radio['selected'] = '';

        $name = $radio['name'];
        unset($radio['name']);
        $selected = $radio['selected'];
        unset($radio['selected']);
    ...
    function makeselect($select) {
        ...
        $emptyoption = $select['emptyoption'];
        unset($select['emptyoption']);
    ...
    function isgoodipaddress($ip) {
        ...
        if (count($parts) <> 4)
            return false;
        if ($parts[0] > 256 or $parts[0] < 0)
            return false;
        if ($parts[1] > 256 or $parts[1] < 0)
            return false;
        if ($parts[2] > 256 or $parts[2] < 0)
            return false;
        if ($parts[3] > 256 or $parts[3] < 0)
            return false;
        return true;
    }
