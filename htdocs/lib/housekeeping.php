...
function isgoodid($wk) {
    if (!$wk)
        return 0; //empty or zero
    if (strlen($wk) > 6)
        return 0;  //more than 999,999
    if (preg_match("/[^\d]/", $wk))
        return 0;  //contains non digits
    return 1;
}
...
function getpost($nm) {
    ...
    if (strlen($wk) > 100000)
        bail("Form variable ($nm) exceeds 100K");
...
function getpostarray($wk) {
    ...
    foreach (array_merge($_GET, $_POST) as $key => $value) {
        if (preg_match("/^$wk(\d+)$/", $key, $m) and $value)
            $myarray[$m[1]] = getpost($wk . $m[1]);
    }
    return $myarray;
}
...
function codeerror($msg, $file, $line) {
    ...
    //db error?
    if ($db and $db->error)
        $cdb = "SQL Error: " . $db->error . "\n";
    elseif (mysql_error())
        $cdb = "SQL Error: " . mysql_error() . "\n";
    else
        $cdb = '';
    if ($db->last_query)
        $cdb2 = "SQL Query: " . $db->last_query . "\n";
    else
        $cdb2 = '';
