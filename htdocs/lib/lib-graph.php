...
class Graph {
    ...
    function prep() {
        ...
        for ($i = 0; $i < $numygrids + 1; $i++) {
            ...
            if ($i % $this->settings['yaxislabelfreq'] == 0)
                ...
                $color = $this->settings['grid_color_minor'];
            $opts = array('color' => $color, 'linewidth' => $tickwidth, 'smooth' => false);
            $this->draw->line($x1, $y1, $x2, $y2, $opts);
        }
    ...
    function drawlineplot($n) {
        ...
        foreach ($ydata2 as $k => $v) {
            if ($v == NULL)
                continue;
            if ($lastgood <> NULL and $v == $lastgood and isset($aheads[$k]) and $v == $aheads[$k])
                $ydata2[$k] = NULL;
            $lastgood = $v;
        }
        ...
        if ($this->settings['usedatapoints']) {
            for ($i = 0; $i < count($xdata); $i++) {
                if (!isset($ydata[$i]))
                    continue; //skip if invalid or null
    ...
    function check_data($data) {
        ...
        if (!is_array($data))
            $errors[] = "Data missing";
        elseif (!isset($data[0]) or ! is_array($data[0]))
            $errors[] = "X data missing";
        ...
        elseif ($npoints) {
            ...
            foreach ($data as $k => $a) {
                //required options
                if (!isset($a['name']) or ! isset($a['color']) or ! isset($a['linewidth']))
                    $errors[] = "Y data $k incomplete";
                elseif (!isset($a['type']) or ! in_array($a['type'], array('line', 'stepline', 'bar')))
                    $errors[] = "Y data $k type bad";
                elseif (!isset($a['linewidth']) or (int) $a['linewidth'] < 1)
                    $errors[] = "Y data $k line bad";
                elseif (!isset($a['color']) or ! trim($a['color']))
                    $errors[] = "Y data $k incomplete";
                elseif (!isset($a['data']) or ! is_array($a['data']))
                    $errors[] = "Y data $k missing";
                elseif (count($a['data']) <> $npoints)
                    $errors[] = "Y data $k invalid";
                else {

                    //convert any named colors
                    if ($a['color'] == 'auto') {
                        $color = array_shift($autocolors);
                        array_shift($autocolors);
                    } elseif (isset($this->colors[trim($a['color'])]))
                        $color = $this->colors[trim($a['color'])];
                    elseif ($this->draw->convertcolor(trim($a['color'])))
                        $color = trim($a['color']);
                    else
                        $errors[] = "Y data $k color bad";
    ...
    function check_params($params) {
        ...
        if (!isset($params['title']))
            $errors[] = "Graph title missing";
        if (!isset($params['scale']))
            $errors[] = "Graph scale array empty";
        elseif ((float) $params['scale'] < 1 or (float) $params['scale'] > 10)
            $errors[] = "Graph scale array invalid";
        if (!isset($params['downsample']))
            $errors[] = "Graph downsample missing";
        elseif ((int) $params['downsample'] < 1 or (int) $params['downsample'] > 1000)
            $errors[] = "Graph downsample invalid";

        if (!isset($params['size']))
            $errors[] = "Graph size array empty";
        elseif (!isset($params['size'][0]) or ! isset($params['size'][1]))
            $errors[] = "Graph size array incomplete";
        elseif ((int) $params['size'][0] < 10 or (int) $params['size'][1] < 10)
            $errors[] = "Graph size too small";
        elseif ((int) $params['size'][0] > 2000 or (int) $params['size'][1] > 2000)
            $errors[] = "Graph size too large";
        elseif (((int) $params['size'][0] * (int) $params['size'][1] * pow((float) $params['scale'], 2)) >= 8000000)
            $errors[] = "Graph size way too large";
        else
            $sizeok = true;

        //margins
        if (!isset($params['margins']))
            $errors[] = "Margins array missing";
        elseif (!isset($params['margins'][0]) or ! isset($params['margins'][1]))
            $errors[] = "Margins array incomplete";
        elseif (!isset($params['margins'][2]) or ! isset($params['margins'][3]))
            $errors[] = "Margins array incomplete";
        elseif ((int) $params['margins'][0] < 10 or (int) $params['margins'][1] < 10)
            $errors[] = "Margins too small";
        elseif ((int) $params['margins'][0] > 2000 or (int) $params['margins'][1] > 2000)
            $errors[] = "Margins too large";
        elseif (!$sizeok) {
            
        } //skip in bounds if size borked
        elseif (((int) $params['margins'][0] + (int) $params['margins'][2]) > (int) $params['size'][1])
            $errors[] = "Margins too large for canvas";
        elseif (((int) $params['margins'][1] + (int) $params['margins'][3]) > (int) $params['size'][0])
            $errors[] = "Margins too large for canvas";

        //legend
        if (!isset($params['showlegend']))
            $errors[] = "Show legend missing";
        elseif (!$params['showlegend']) {
            
        } elseif (!isset($params['legend_pos']))
            $errors[] = "Legend array incomplete";
        elseif (!isset($params['legend_pos'][0]) or ! isset($params['legend_pos'][1]))
            $errors[] = "Legend pos array incomplete";
        elseif (!$sizeok) {
            
        } elseif (abs((int) $params['legend_pos'][0]) > (int) $params['size'][0])
            $errors[] = "Legend pos too large for canvas";
        elseif (abs((int) $params['legend_pos'][1]) > (int) $params['size'][1])
            $errors[] = "Legend pos too large for canvas";

        //note
        if (!isset($params['shownote']))
            $errors[] = "Show note missing";
        elseif (!$params['shownote']) {
            
        } elseif (!isset($params['note_pos']))
            $errors[] = "note array incomplete";
        elseif (!isset($params['note_pos'][0]) or ! isset($params['note_pos'][1]))
            $errors[] = "note pos array incomplete";
        elseif (!$sizeok) {
            
        } elseif (abs((int) $params['note_pos'][0]) > (int) $params['size'][0])
            $errors[] = "note pos too large for canvas";
        elseif (abs((int) $params['note_pos'][1]) > (int) $params['size'][1])
            $errors[] = "note pos too large for canvas";

        //fonts
        if (!isset($params['fontfolder']))
            $errors[] = "Font folder missing";
        elseif (!isset($params['fontcolor']))
            $errors[] = "Font color missing";
        elseif (!isset($params['fontsize']))
            $errors[] = "Font size missing";
        elseif (!isset($params['fontfile']))
            $errors[] = "Font file missing";
        elseif (!file_exists((string) $params['fontfolder']))
            $errors[] = "Font folder not found";
        elseif (!file_exists((string) $params['fontfolder'] . (string) $params['fontfile']))
            $errors[] = "Font file not found";
        elseif (!isset($params['fontfilebold'])) {
            
        } elseif (!file_exists((string) $params['fontfolder'] . (string) $params['fontfilebold']))
            $errors[] = "Bold Font file not found";

        //canvas colors
        if (!isset($params['border_color']))
            $errors[] = "Border color missing";
        elseif (!isset($this->colors[(string) $params['border_color']]) and ! $this->testcolorgood((string) $params['border_color']))
            $errors[] = "Border color bad";
        if (!isset($params['grid_color_minor']))
            $errors[] = "Major grid color missing";
        elseif (!isset($this->colors[(string) $params['grid_color_minor']]) and ! $this->testcolorgood((string) $params['grid_color_minor']))
            $errors[] = "Major grid color bad";
        if (!isset($this->colors[(string) $params['grid_color_major']]) and ! $this->testcolorgood((string) $params['grid_color_major']))
            $errors[] = "Minor grid color bad";
        elseif (!isset($params['grid_color_major']))
            $errors[] = "Minor grid color missing";

        //x params
        if (!isset($params['xaxistitle']))
            $errors[] = "X xaxistitle missing";
        if (!isset($params['xmode']))
            $errors[] = "X xmode missing";
        elseif (!in_array($params['xmode'], array('betw', 'adj')))
            $errors[] = "X xmode invalid";
        if (!isset($params['xusemajorgrid']))
            $errors[] = "X usemajorgrid missing";
        if (!isset($params['xintervalmajor']))
            $errors[] = "X xintervalmajor missing";
        elseif ((int) $params['xintervalmajor'] < 1)
            $errors[] = "X xintervalmajor invalid";
        if (!isset($params['xuseminorgrid']))
            $errors[] = "X xuseminorgrid missing";
        elseif ($params['xuseminorgrid']) {
            if (!isset($params['xqtyminorgrids']))
                $errors[] = "X xqtyminorgrids missing";
            elseif ((int) $params['xqtyminorgrids'] < 2)
                $errors[] = "X xqtyminorgrids invalid";
            elseif ((int) $params['xqtyminorgrids'] > (int) $params['xintervalmajor'])
                $errors[] = "X xqtyminorgrids invalid";
        }

        //y params
        if (!isset($params['yaxistitle']))
            $errors[] = "Y yaxistitle missing";
        if (!isset($params['yusemajorgrid']))
            $errors[] = "Y yusemajorgrid missing";
        if (!isset($params['ymode']))
            $errors[] = "Y ymode missing";
        elseif (!in_array($params['ymode'], array('fit', 'exact', 'auto')))
            $errors[] = "Y ymode invalid";
        elseif ($params['ymode'] <> 'auto') {
            if (!isset($params['yextents']) or ! is_array($params['yextents']))
                $errors[] = "Y yextents missing";
            elseif (!isset($params['yextents'][0]) or ! isset($params['yextents'][1]))
                $errors[] = "Y yextents incomplete";
        }
        if (!isset($params['yusemajorgrid']))
            $errors[] = "Y yusemajorgrid missing";
        if (!isset($params['yqtymajorgrids']))
            $errors[] = "Y yqtymajorgrids missing";
        elseif ((int) $params['yqtymajorgrids'] < 1)
            $errors[] = "Y yqtymajorgrids invalid";

        if (!isset($params['yuseminorgrid']))
            $errors[] = "Y yuseminorgrid missing";
        elseif ((int) $params['yqtyminorgrids'] < 2)
            $errors[] = "Y yqtyminorgrids invalid";
        if (!preg_match("/^decimal\(\d\)$/", trim($params['yaxislabelspec'])))
            $errors[] = "Y yaxislabelspec invalid";

        //series params
        if (!isset($params['usedatapoints']))
            $errors[] = "Series usedatapoints missing";
        elseif ($params['usedatapoints']) {
            if (!isset($params['datapointshape']))
                $errors[] = "Series datapointshape missing";
            elseif (!isset($params['datapointsize']))
                $errors[] = "Series datapointsize missing";
        }
        if (!isset($params['usedatalabels']))
            $errors[] = "Series usedatalabels missing";
        elseif ($params['usedatalabels']) {
            if (!isset($params['datalabelinterval']))
                $errors[] = "Series datalabelinterval missing";
            elseif (!isset($params['datalabelspec']))
                $errors[] = "Series datalabelspec missing";
            elseif (!preg_match("/^decimal\(\d\)$/", trim($params['datalabelspec'])))
                $errors[] = "Y datalabelspec invalid";
        }
        if (!isset($params['linejoinmethod']))
            $errors[] = "Series linejoinmethod missing";
        elseif (!in_array($params['linejoinmethod'], array('round', 'angle', 'square', 'none')))
            $errors[] = "Y linejoinmethod invalid";
        ...
        if ((int) $params['legend_pos'][0] < 0)
            $params['legend_pos'][0] = (int) $params['size'][0] + (int) $params['legend_pos'][0];
        if ((int) $params['legend_pos'][1] < 0)
            $params['legend_pos'][1] = (int) $params['size'][1] + (int) $params['legend_pos'][0];
        if ((int) $params['note_pos'][0] < 0)
            $params['note_pos'][0] = (int) $params['size'][0] + (int) $params['note_pos'][0];
        if ((int) $params['note_pos'][1] < 0)
            $params['note_pos'][1] = (int) $params['size'][1] + (int) $params['note_pos'][0];
